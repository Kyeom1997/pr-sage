...
import { annotatePatch, commentableLines, validateFindings } from "../src/diff.js";
...
describe("annotatePatch", () => {
  it("numbers right-side lines and leaves deletions unnumbered", () => {
    const annotated = annotatePatch(PATCH).split("\n");
    expect(annotated[1]).toBe("    1  const a = 1;");
    expect(annotated[2]).toBe("      -const b = 2;");
